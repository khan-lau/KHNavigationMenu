...
@property (nonatomic, strong) UIView * blurViewBackground;
@property (nonatomic, strong) UIView * containerBackground;
...
//            UIVisualEffectView *v = [[UIVisualEffectView alloc] initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleDark]];
        ...
        _blurViewBackground = self.blurView;
        _containerBackground = self.container;
...
- (void) setContainerTransparent:(BOOL) isTransparent{
    ...
    if (isTransparent) {
        ...
        self.blurView = _blurViewBackground;
        self.container = _containerBackground;
...
- (void)layoutSubviews {
    ...
    if (self.state <= RWBlurPopoverViewStateShowing) {
        if (self.isPopover) {
            self.contentView.frame = CGRectMake((CGRectGetWidth(self.container.bounds) - self.contentSize.width) / 2.0, (CGRectGetHeight(self.container.bounds) - self.contentSize.height) / 2.0, self.contentSize.width, self.contentSize.height );

        }else{
            self.contentView.frame = CGRectMake((CGRectGetWidth(self.container.bounds) - self.contentSize.width) / 2.0, self.offsetY, self.contentSize.width, self.contentSize.height );
        }
    }
}
...
- (void)animatePresentation {
    [self animatePresentationWithCompletion:nil];
}
...
- (void)animateDismissalWithCompletion:(void (^)(void))completion {
    ...
    gravityBehavior.magnitude = (self.isPopover) ? 4 : -4;
